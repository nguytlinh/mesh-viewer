...
bool zoom = false, mouse_rotate = false;
float lastX, lastY;
...
static void cursor_position_callback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    if (mouse_rotate) {
        if (zoom) {
            float change = (float)ypos - lastY;
            dist += change;
        }
        else {
            float changeX = (float)xpos - lastX;
            float changeY = (float)ypos - lastY;
            azimuth += changeX;
            elevation += changeY;
        }
        lastX = (float)xpos;
...
int main(int argc, char** argv)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float scalef = 2.0f / max_dimension;
        glm::mat4 scale_matrix = glm::scale(glm::mat4(1.0f), glm::vec3(scalef));
